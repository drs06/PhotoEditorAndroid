...
open class PermissionHandlerWithUI(
    ...
    private fun showDialog(normalDialog: Boolean) {
        ...
        dialog = MaterialAlertDialogBuilder(getContext(), getDialogStyle())
            .setTitle(title)
            .setMessage(message)
            .setNegativeButton(if (normalDialog) denyButtonText else notNowButtonText) { d, _ ->
                hideDialog()
            }
            .setPositiveButton(if (normalDialog) allowButtonText else appSettingsButtonText) { d, _ ->
                if (normalDialog) {
